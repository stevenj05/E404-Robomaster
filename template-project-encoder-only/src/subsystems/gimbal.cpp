...
void Gimbal::calibrateEncoderToGyro()
{
    ...
    // Set baseline to the raw reading - this makes "forward turret at calibration" = 0°
    // Relative movement from this point will be tracked correctly
...
void Gimbal::update()
{
    ...
    if (beybladeMode)
    {
        ...
        gimbalYawTargetPos += -yawInput * 120.0f;
    ...
    else
    {
        ...
        else
        {
            ...
            if (!pidYawActive)
            {
                ...
                yawMotor->setDesiredOutput(static_cast<int32_t>(-yawInput * 7980));
            }
            else
            {
                // After 3 seconds: activate PID for position holding
                // Update target position based on joystick input
                gimbalYawTargetPos += -yawInput * 7.2f;  // 40% reduction from original
    ...
    else
    {
        // After 3 seconds: activate PID for position holding
        // Update target position based on joystick input
        gimbalPitchTargetPos += pitchInput * 3.0f;  // Pitch direction inverted
...
float Gimbal::getYawAngleDegrees() const
{
    ...
    // Account for 1:1 gear ratio: direct drive
